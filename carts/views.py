    ...
    # if the is not authenticated
        ...
        if request.method == 'POST':
            for item in request.POST:
                ...
                try:
                    variant = Variant.objects.get(product=product, variation_category__iexact=key, variation_value__iexact=value)
                    product_variant.append(variant)
                except:
                    pass
        try:
...
def checkout(request, total=0, quantity=0, sending_cost=0, cart_items=None):
    try:
        tax = 0
        grand_total = 0
        if request.user.is_authenticated:
        ...
        for cart_item in cart_items:
            total += round((cart_item.product.price * cart_item.quantity) - (
                        (21 * cart_item.product.price * cart_item.quantity) / 100), 2)
            quantity += cart_item.quantity
            tax = round((21 * cart_item.product.price * cart_item.quantity) / 100, 2)
            if total <= 50:
                sending_cost = round(Decimal(2.99), 2)
